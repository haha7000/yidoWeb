#!/bin/bash
# AWS CLI를 사용해 Mac 인스턴스 생성 및 설정 스크립트

echo "🍎 AWS Mac 인스턴스 생성 및 설정 시작..."
...
# 설정 변수들
INSTANCE_NAME="DbTest-Mac-Instance"
...
if aws ec2 describe-security-groups --group-names $SECURITY_GROUP_NAME &> /dev/null; then
    ...
    SECURITY_GROUP_ID=$(aws ec2 create-security-group \
        ...
        --description "Security group for DbTest Mac instance" \
        --vpc-id $VPC_ID \
        --query 'GroupId' \
        --output text)
...
echo ""
echo "2. 프로젝트 파일 업로드:"
echo "   ${BLUE}scp -i ${KEY_PAIR_NAME}.pem -r ./ ec2-user@$PUBLIC_IP:~/DbTest/${NC}"
echo ""
echo "3. 또는 Git 클론:"
echo "   ${BLUE}ssh -i ${KEY_PAIR_NAME}.pem ec2-user@$PUBLIC_IP${NC}"
echo "   ${BLUE}git clone your-repository-url DbTest${NC}"
echo ""
echo "4. 설정 스크립트 실행:"
echo "   ${BLUE}cd DbTest && chmod +x setup-mac.sh && ./setup-mac.sh${NC}"
echo ""
...
if [[ $AUTO_DEPLOY =~ ^[Yy]$ ]]; then
    ...
    check_error "프로젝트 업로드 실패"
    
    # 원격에서 압축 해제 및 설정
    ssh -i ${KEY_PAIR_NAME}.pem ec2-user@$PUBLIC_IP << 'EOF'
        tar -xzf dbtest-project.tar.gz -C DbTest/
        cd DbTest
...
EOF
    ...
    echo "🔗 서비스 접속: http://$PUBLIC_IP:8000"
fi

# 인스턴스 정보를 파일로 저장
cat > mac-instance-info.txt << EOF
AWS Mac Instance Information
============================
...
success_msg "인스턴스 정보가 'mac-instance-info.txt'에 저장되었습니다"
