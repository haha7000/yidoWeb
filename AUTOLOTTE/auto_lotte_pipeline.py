...
sys.path.append('/Users/ec2-user/yido/yidoweb')
from execl_test import process_lotte_excel, convert_to_lotte_db_schema, generate_sqlalchemy_dtypes
...
class LotteAutoPipeline:
    ...
    def step2_convert_data_types(self, verbose: bool = True) -> bool:
        """
        2단계: 데이터 타입 변환
        ...
        try:
            # 데이터 타입 변환
            self.converted_df, self.sqlalchemy_dtypes, success_rate = process_lotte_excel(
                self.downloaded_file, verbose=verbose
            )
            
            if self.converted_df is not None and success_rate >= 90:
                logger.info(f"✅ 데이터 타입 변환 완료 (성공률: {success_rate:.1f}%)")
                return True
            else:
                logger.error(f"❌ 데이터 타입 변환 실패 (성공률: {success_rate:.1f}%)")
                return False
    ...
    def step3_upload_to_db(self, batch_size: int = 1000) -> bool:
        """
        3단계: DB 자동 업로드
        ...
        Returns:
            bool: 업로드 성공 여부
        """
        logger.info("🗄️ 3단계: DB 자동 업로드 시작")
        ...
        try:
            ...
            # 배치 단위로 데이터 삽입
            total_rows = len(self.converted_df)
            inserted_count = 0
            
            logger.info(f"📥 총 {total_rows:,}건의 데이터를 배치 크기 {batch_size}로 업로드 중...")
            
            for i in range(0, total_rows, batch_size):
                batch_df = self.converted_df.iloc[i:i+batch_size]
                ...
                for _, row in batch_df.iterrows():
                    ...
                    # 컬럼별로 데이터 할당
                    for col in self.converted_df.columns:
    ...
    def run_full_pipeline(self, user_id: str = 'T301912', password: str = 'huixin210@', 
        ...
        try:
            ...
            # 2단계: 데이터 타입 변환
            if not self.step2_convert_data_types(verbose):
                logger.error("❌ 2단계 실패로 파이프라인 중단")
                return False
            
            # 3단계: DB 업로드
            if not self.step3_upload_to_db():
                logger.error("❌ 3단계 실패로 파이프라인 중단")
                return False
            ...
            logger.info(f"📊 처리된 데이터: {len(self.converted_df):,}건")
...
def main():
    ...
    try:
        success = pipeline.run_full_pipeline(user_id, password, verbose=True)
