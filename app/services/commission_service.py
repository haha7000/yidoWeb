...
class CommissionService:
    ...
    def _get_commission_rate(self, sales_date, user_id: int, store_branch: str, 
        ...
        # 최우선순위: exempt_brands 확인 (카테고리+브랜드 일치하면 수수료 0%)
        if brand and category:
            ...
            exempt_result = self.session.execute(text(exempt_query), {
            ...
            if exempt_result:
                logger.info(f"브랜드 {brand}, 카테고리 {category}는 수수료 제외 브랜드입니다. 수수료 0% 적용")
                return Decimal('0.0')
        
        # 1순위: product_code로 item_fees 조회 (카테고리+브랜드+상품코드 모두 일치)
        if product_code and brand and category:
            item_query = """
            ...
            if item_result:
                logger.info(f"상품코드 {product_code}, 브랜드 {brand}, 카테고리 {category}로 수수료율 발견: {item_result[0]}")
                return item_result[0]
        
        # 2순위: brand로 brand_fees 조회 (카테고리+브랜드 일치)
        if brand and category:
            brand_query = """
            SELECT commission_rate 
            FROM brand_fees 
            ...
            if brand_result:
                logger.info(f"브랜드 {brand}, 카테고리 {category}로 수수료율 발견: {brand_result[0]}")
                return brand_result[0]
        
        # 3순위: category로 category_fees 조회 (카테고리만 일치)
        if category:
            # 카테고리를 7개 고정 구분으로 매핑
            fee_type = self._map_category_to_fee_type(category, duty_free_type)
            ...
            if fee_type:
                ...
                if category_result:
                    logger.info(f"카테고리 {category} → 구분 {fee_type}로 수수료율 발견: {category_result[0]}")
                    return category_result[0]
        
        logger.warning(f"수수료율을 찾을 수 없습니다: 상품코드={product_code}, 브랜드={brand}, 카테고리={category}")
        return None
    
    def _map_category_to_fee_type(self, category: str, duty_free_type: str = None) -> Optional[str]:
        """카테고리 코드를 7개 고정 구분으로 매핑"""
        if not category:
            return None
        ...
        # 카테고리별 매핑 규칙
        if category == "COS AND PFM":
            # TODO: 나중에 로컬/수입 구분 추가 예정
            # 현재는 기본적으로 수입 화장품으로 처리
            logger.info(f"COS AND PFM 카테고리를 '수입 화장품'으로 매핑 (로컬/수입 구분 미적용)")
            return "수입 화장품"
            ...
            # FASHION, JEW AND WATCH, LEATHER를 제외한 모든 카테고리는 일반
            if category in ["FASHION", "JEW AND WATCH", "LEATHER"]:
                logger.info(f"카테고리 {category}는 일반 구분에서 제외됨")
                return None
            else:
                # EYEWEAR, LEATHER, ACC and GIFT 등등은 일반
                logger.info(f"카테고리 {category}를 '일반' 구분으로 매핑")
                return "일반"
